...
function TimelineLabel({ align, date, label }: TimelineLabelProps) {
  return (
    <div className="flex flex-1 flex-col gap-1" style={{ alignItems: align }}>
      <span className="text-[0.78rem] font-semibold text-ink dark:text-teal">{date}</span>
      <span>{label}</span>
    </div>
  )
}
...
export function TimelineCard({ window }: TimelineCardProps) {
  return (
    <Card title="Timeline">
      ...
      <div className="-mt-2 flex justify-between text-xs text-muted-light dark:text-muted-dark">
      ...
      <div className="mt-5 flex flex-wrap gap-3.5 text-[0.78rem] text-muted-light dark:text-muted-dark">
